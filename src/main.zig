...
const MAX_SRC_BYTES = std.math.maxInt(u32);
...
pub fn main() anyerror!void {
    ...
    // TODO: dont alloc if check_format disabled
    var check_format_buffer = std.ArrayList(u8).init(arena_allocator);
    defer check_format_buffer.deinit();
    for (files) |file| {
        ...
        try lint(file, allocator, analyzer, &check_format_buffer, true);
    }
    // if (fault_count >= 256) {
...
fn lint(
    ...
    const file = std.fs.cwd().openFile(file_name, .{}) catch |err| {
        switch (err) {
            ...
            error.IsDir => {
                return try walk_directory(file_name, alloc, analyzer, check_format_buffer);
            },
    ...
    const stat = try file.stat(); // TODO: is .stat() faster?
    // var fault_count: u64 = 0;
    const is_dir = (stat.kind == .directory);
    if (is_dir) {
        return walk_directory(file_name, alloc, analyzer, check_format_buffer);
    }
    if (!is_top_level) {
    ...
    return lint_file(alloc, file, file_name, stat.size, analyzer, check_format_buffer);
}
...
fn lint_file(
    ...
    var faults = try analyzer.analyze(alloc, file_name, ast, check_format_buffer);
    _ = faults;
    // defer faults.deinit();
...
fn walk_directory(
    ...
    // var fault_count: u64 = 0;
    while (try iterable.next()) |entry| {
        const full_name = try std.fs.path.join(alloc, &[_][]const u8{ file_name, entry.name });
        defer alloc.free(full_name);
        // TODO: possible performance gains by having a lint_file vs lint_directory function and jumping straight there from here?
        if (entry.kind == .directory) {
            try walk_directory(full_name, alloc, analyzer, check_format_buffer);
        } else {
            if (!std.mem.endsWith(u8, entry.name, ".zig")) continue;
            const file = try dir.dir.openFile(entry.name, .{});
            defer file.close();
            try lint_file(alloc, file, full_name, null, analyzer, check_format_buffer);
        }
    }
}
